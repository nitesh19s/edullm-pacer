 ...
 * Data is persisted in the Express/SQLite backend via the API client.
 */

class ResearchFeaturesManager {
    constructor(database, apiClient) {
        this.database  = database;  // kept for backward-compat, not used
        this._client   = apiClient || null;
        this.chartManager        = null;
        this.progressionViz      = null;
        this.gapCrossSubjectViz  = null;
        this.initialized         = false;
        this.currentGapReport    = null;
        this.currentCrossSubjectAnalysis = null;
    }

    get apiClient() {
        return this._client || window.eduLLM?.apiClient || null;
    }

    async initialize() {
        if (this.initialized) return;
        console.log('Initializing Research Features Manager (API mode)...');
    ...
    setupEventListeners() {
        ...
        bind('viewProgressionBtn',     this.viewProgression);
        bind('exportProgressionBtn',   this.exportProgression);
        bind('refreshProgressionBtn',  this.refreshProgression);
        bind('analyzeGapsBtn',         this.analyzeGaps);
        bind('exportGapReportBtn',     this.exportGapReport);
        bind('analyzeCrossSubjectBtn', this.analyzeCrossSubject);
        bind('exportCrossSubjectBtn',  this.exportCrossSubject);
        bind('refreshCrossSubjectBtn', this.analyzeCrossSubject);
    }

    // ── public API: track interaction ─────────────────────────────────────────

    async trackLearningInteraction(interaction) {
        const client = this.apiClient;
        if (!client) return;
        try {
            await client.trackInteraction({ studentId: 'default_student', ...interaction });
        } catch (error) {
    ...
    async viewProgression() {
        const client = this.apiClient;
        if (!client) { this.showError('Backend not connected'); return; }
        try {
            const resp = await client.getProgressionAnalytics('default_student');
            this.updateProgressionUI(this._normalizeProgression(resp.data));
        } catch (error) {
            if (error.statusCode === 404) {
                // No data yet — show empty-state UI
                this.updateProgressionUI(this._normalizeProgression({
                    mastery: { mastered: [], learning: [], struggling: [] },
                    metrics: { currentLevel: 0, learningVelocity: 0, successRate: 0 },
                    learningPath: []
                }));
            } else {
    ...
    _normalizeProgression(data) {
        const lvl = data.metrics?.currentLevel || 0;
        const currentLevel = lvl >= 4 ? 'advanced' : lvl >= 2 ? 'intermediate' : 'beginner';
        return {
            overview: {
                currentLevel,
                learningVelocity: data.metrics?.learningVelocity || 0,
                retentionRate:    (data.metrics?.successRate || 0) / 100
            },
            mastery:      data.mastery || { mastered: [], learning: [], struggling: [] },
            learningPath: (data.learningPath || []).map(e => ({
                ...e, masteryLevel: e.masteryLevel ?? 0
            })),
            recommendations: []
        };
    }
        ...
        if (mastery.mastered.length > 0) {
            mastery.mastered.slice(0, 5).forEach(concept => {
                html += `<div class="concept-item mastered">
                    ...
                    <span class="mastery-badge">${concept.masteryLevel.toFixed(0)}%</span>
                </div>`;
            });
        ...
        learningPath.forEach(entry => {
            ...
            html += `<div class="path-entry">
                ...
                <div class="path-content">
                    ...
                    <p class="text-muted">${date} • Mastery: ${(entry.masteryLevel || 0).toFixed(0)}%</p>
                </div>
            </div>`;
        });
        html += '</div>';
    ...
    async exportProgression() {
        ...
        if (!client) { this.showError('Backend not connected'); return; }
        try {
            const resp = await client.getProgression('default_student');
            this.downloadFile(JSON.stringify(resp.data, null, 2), 'progression-data.json', 'application/json');
        } catch (error) {
    ...
    async analyzeGaps() {
        ...
        try {
            ...
            const resp = await client.analyzeCurriculumGaps({
                studentId:     'default_student',
                targetGrade,
                targetSubject: targetSubject === 'all' ? null : targetSubject
            });

            const report = this._normalizeGapReport(resp.data);
            this.updateGapAnalysisUI(report);
            this.currentGapReport = report;
        } catch (error) {
            if (error.statusCode === 404) {
                this.showError('No progression data found. Use the chat to interact with content first.');
            } else {
    ...
    _normalizeGapReport(data) {
        ...
        return {
            coverage: {
                ...
                mastered:           cov.mastered   || 0,
                coveragePercentage: total > 0 ? ((cov.covered || 0) / total) * 100 : 0
            },
            gaps: {
                concepts: (data.gaps?.concepts || []).map(g => ({
                    ...
                    severity:     g.severity,
                    isCritical:   g.severity > 70
                })),
                total: data.gaps?.total || 0
            },
            recommendations: (data.recommendations || []).map(r => ({
                priority:    r.priority >= 70 ? 'high' : r.priority >= 40 ? 'medium' : 'low',
                title:       r.conceptName,
                description: r.reason || '',
                action:      'Review and practice this concept'
            }))
        };
    }
    ...
    renderGapsList(gaps) {
        ...
        gaps.forEach(gap => {
            const criticalBadge  = gap.isCritical ? '<span class="badge badge-danger">Critical</span>' : '';
    ...
    async exportGapReport() {
        if (!this.currentGapReport) { this.showError('Please analyze gaps first'); return; }
        this.downloadFile(
            JSON.stringify(this.currentGapReport, null, 2),
            'curriculum-gap-report.json',
            'application/json'
        );
    }
    ...
    async analyzeCrossSubject() {
        ...
        if (!client) { this.showError('Backend not connected'); return; }
        try {
            const resp     = await client.analyzeCrossSubject({ studentId: 'default_student' });
            const analysis = this._normalizeCrossSubject(resp.data);
    ...
    _normalizeCrossSubject(data) {
        const sp = data.subjectPerformance || {};

        // Normalize each subject entry: backend uses `successRate`, UI expects `averageSuccessRate`
        const subjectPerformance = {};
        Object.entries(sp).forEach(([key, s]) => {
            subjectPerformance[key] = { ...s, averageSuccessRate: s.successRate || s.averageSuccessRate || 0 };
        });

        // Backend patterns.strengths/weaknesses are arrays of strings; UI expects {subject, averageMastery}
        const toObj = arr => (arr || []).map(name => ({
            subject:       name,
            averageMastery: subjectPerformance[name]?.averageMastery || 0
        }));

        return {
            subjectPerformance,
            patterns: {
                strengths:  toObj(data.patterns?.strengths),
                weaknesses: toObj(data.patterns?.weaknesses)
            },
            correlations:              [],
            interdisciplinaryInsights: { totalInterdisciplinary: 0, concepts: [] },
    ...
    renderStrengths(strengths) {
        ...
        strengths.forEach(s => {
            html += `<li><strong>${s.subject}</strong>: ${s.averageMastery.toFixed(1)}% mastery</li>`;
        });
        html += '</ul>';
        return html;
    }
    ...
    renderWeaknesses(weaknesses) {
        ...
        weaknesses.forEach(w => {
            html += `<li><strong>${w.subject}</strong>: ${w.averageMastery.toFixed(1)}% mastery</li>`;
        });
        html += '</ul>';
        return html;
    }
    ...
    async exportCrossSubject() {
        ...
        this.downloadFile(
            JSON.stringify(this.currentCrossSubjectAnalysis, null, 2),
            'cross-subject-analysis.json',
            'application/json'
        );
    }
